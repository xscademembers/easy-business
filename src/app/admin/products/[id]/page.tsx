    ...
    try {
      const body = {
        ...
        variants: variants
          ...
          .map((v) => ({ ...v, price: v.price ? parseFloat(v.price) : undefined })),
      };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="card space-y-4">
          <h2 className="font-semibold" style={{ color: 'var(--text-primary)' }}>Basic Details</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              <select value={form.category} onChange={(e) => { setForm({ ...form, category: e.target.value }); setCategoryFields({}); }} className="input-field">
                {CATEGORIES.map((c) => (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {dynamicFields.map((field) => (
                <div key={field.key}>
                  ...
                  <input type={field.type} value={categoryFields[field.key] || ''} onChange={(e) => setCategoryFields({ ...categoryFields, [field.key]: e.target.value })} className="input-field" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="card space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold" style={{ color: 'var(--text-primary)' }}>Variants</h2>
            <button type="button" onClick={addVariant} className="text-sm font-medium flex items-center gap-1" style={{ color: 'var(--accent)' }}>
              <Plus size={16} /> Add
            </button>
          </div>
          {variants.map((v, i) => (
